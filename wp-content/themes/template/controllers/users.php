...
if ( ! class_exists('TDF_Users') ){
  ...
  class TDF_Users {
    ...
    public function __construct() {
      ...
      add_action( 'wp_ajax_tdf_send_reset_password',array( $this, 'send_reset_pwd' ) );
      add_action( 'wp_ajax_nopriv_tdf_send_reset_password',array( $this, 'send_reset_pwd' ) );
          ...
          if($_GET["action"]=="reset_pwd"){
            ...
            if(isset($user_data)){
              ...
              if (!empty($reset_key) && !empty($user_data)) {
                ...
                $redirect_to = get_permalink(PAGE_ID_ACCOUNT_EDIT)."?action=reset_pass";
                wp_safe_redirect($redirect_to);
                exit();
                // }
              } else {
                $redirect_to = get_permalink(PAGE_ID_ACCOUNT_EDIT)."?action=invalid_reset_key";
                wp_safe_redirect($redirect_to);
                exit();
              }
            }
    ...
    function send_reset_pwd(){
      ...
      $reset_link = get_bloginfo("url")."?action=reset_pwd&key=$key&login=" . rawurlencode($user_login);

      $email = new TDF_Email_Model;
      $email->send_email("forgot",$user_data->ID,array("user_login"=>$user_data->user_login,"user_name"=>ucfirst(get_user_meta($user_data->ID, "first_name", true)) . " " . ucfirst(get_user_meta($user_data->ID, "last_name", true)),"reset_link"=>$reset_link));
    ...
    public function save(){
      ...
      if($user_id){
        if($_REQUEST["join_newsletter"]){
          $the_email = $_REQUEST["user_core_email"];

          $apiKey = "cba68d4965e2d7d6170c21392a1f7e98-us20";
          $listID = "13c7288b36";

          $memberID = md5(strtolower($the_email));
        ...
        if(is_wp_error($user_id)){
          ...
          $email = new TDF_Email_Model;
          $email->send_email("register",$user_id,array("user_name"=>ucfirst(get_user_meta($user_data->ID, "first_name", true)) . " " . ucfirst(get_user_meta($user_data->ID, "last_name", true)),"user_login"=>$user_data->user_login,"user_password"=>$_REQUEST["user_core_password"],"confirm_link"=>get_bloginfo("url")."/?confirm_email=".$user_data->user_email));
