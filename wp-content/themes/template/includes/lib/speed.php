...
if ( ! class_exists('TDF_Speed') ){
  ...
  class TDF_Speed {
    ...
    public function __construct($cont) {
      ...
      // $cont = $this->process_images($cont);
      $cont = $this->process_styles($cont);

      echo $cont;
    }

    function process_images($cont){

      $cont = str_replace("<iframe1 src=",'<iframe1 data-src=',$cont);
      $cont = str_replace('.jpg"','" data-ext="jpg"',$cont);
      $cont = str_replace(".jpg'","' data-ext='jpg'",$cont);
      $cont = str_replace('.jpeg"','" data-ext="jpeg"',$cont);
      $cont = str_replace(".jpeg'","' data-ext='jpeg'",$cont);
      $cont = str_replace('.png"','" data-ext="png"',$cont);
      $cont = str_replace(".png'","' data-ext='png'",$cont);
      $cont = str_replace('.gif"','" data-ext="gif"',$cont);
      $cont = str_replace(".gif'","' data-ext='gif'",$cont);
      $cont = str_replace('.svg"','" data-ext="svg"',$cont);
      $cont = str_replace(".svg'","' data-ext='svg'",$cont);


      $cont = str_replace("<img src=",'<img src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src=',$cont);
      $cont = str_replace("</head>","<style>body{opacity:0;transition:0.15s all;transition-delay:0.15s;}</style></head>",$cont);
      $cont = str_replace("</body>","<script>function init_imgs() {var imgDefer = document.getElementsByTagName('img');for (var i = 0; i < imgDefer.length; i++) {if (imgDefer[i].getAttribute('data-src')) {imgDefer[i].setAttribute('src',imgDefer[i].getAttribute('data-src')+'.'+imgDefer[i].getAttribute('data-ext'));}}var imgDefer = document.getElementsByTagName('iframe1');for (var i = 0; i < imgDefer.length; i++) {if (imgDefer[i].getAttribute('data-src')) {imgDefer[i].setAttribute('src',imgDefer[i].getAttribute('data-src')+'.'+imgDefer[i].getAttribute('data-ext'));}}}setInterval(function(){init_imgs();jQuery('body').css('opacity',1);},250);</script></body>",$cont);
      return $cont;
    }
