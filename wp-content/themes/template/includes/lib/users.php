  ...
  class TDF_Users_Model {
    ...
    function get_avatar($avatar, $id_or_email, $size) {
      ...
      if ($avatar_img = wp_get_attachment_image($av, array($size, $size))) {
        ...
        $avatar = "<img alt='image_alt' src='" . get_bloginfo("template_url") . "/assets/images/defaults/no_user.png' class='avatar avatar-{$size} photo avatar-default' height='{$size}' width='{$size}' />";
      }
      return $avatar;
    }
      ...
      if(isset($_SERVER['HTTP_REFERER'])){
        ...
        if (!empty($referrer) && !strstr($referrer, 'wp-login') && !strstr($referrer, 'wp-admin') && $error) {
          if (!strpos($referrer, '?login=failed')) {
            if (!strpos($referrer, '?')) {
              wp_redirect($referrer . '?login1=failed');
            } else {
              wp_redirect($referrer . '&login=failed');
            }
          } else {
    ...
    function get_display_name($user_id,$post_id=0){
      ...
      if ($post_id) {
        ...
        return ucfirst(get_user_meta($user_id, "first_name", true)) . " " . substr(ucfirst(get_user_meta($user_id, "last_name", true)), 0, 1);
      }
    }
    ...
    function get_add_field($name, $type = "", $args = "") {
      ...
      if ($name == "avatar") {
        echo '<input type="file" name="user_core_avatar" id="user_core_avatar" class="' . $args["classes"] . '" /><span></span>';
      }
    ...
    function get_update_field($name, $pid, $type = "", $args = "") {
      ...
      if ($name == "avatar") {
        ...
        echo '<div style="float:left; padding-right:15px;" class="img_max_width">';
        ...
        echo '</div><div style="float:left; width:75%;">';
        echo '<input type="file" name="user_core_avatar" id="user_core_avatar" /><span></span>';
        echo '</div>';
      }
      if ($name == "image_gallery") {
        $images2 = array();
