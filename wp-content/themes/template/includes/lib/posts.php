  ...
  class TDF_Posts_Model {
      ...
      */
      ...
      if (count($my_query->posts)) {
        foreach($my_query->posts as $pid){
          if(intval($pid)>0){
            if(isset($args["get_basic_info"])){
              $items[] = array("post_id"=>$pid,"post_permalink"=>get_permalink($pid),"post_title"=>get_the_title($pid));
            }else{
    ...
    function get_the_post($pid, $args = array()){
      ...
      if($pid){
        ...
        $item["post_content"] = apply_filters("the_content",get_post_field('post_content',$pid));
        $item["post_excerpt"] = get_the_excerpt($pid);
        $item["post_type"] = $post_type;
        $item["post_date"] = strtotime($post->post_date);
    ...
    function generate_query_args($args){
      ...
      if(isset($args["order"])){
        if ($args["order"] == "title_asc") {
          ...
          $query_args['orderby'] = 'comment_count';
          $query_args['order'] = 'DESC';
        } elseif (substr($args["order"], 0, 13) == "meta_asc_num_") {
          $query_args['orderby'] = 'meta_value_num';
          $query_args['meta_key'] = substr($args["order"], 13);
          $query_args['order'] = 'ASC';
        } elseif (substr($args["order"], 0, 14) == "meta_desc_num_") {
          $query_args['orderby'] = 'meta_value_num';
          $query_args['meta_key'] = substr($args["order"], 14);
          $query_args['order'] = 'DESC';
