...
echo "aaaaaaaaaaaa";
            ...
            add_filter( 'tdf_get_snap_price_test', array( $this, 'get_snap_price_info' ), 10, 1);
    ...
    public function get_news($pid=0){
      ...
      if($pid){
        ...
        curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_news.php?company=".$company_code);
      }else{
        curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/news.php");
      }
      ...
      foreach($json["results"]["news"][0]["newsitem"] as $item){
        ...
        if(!$post_id){
          ...
          foreach($topics_pieces as $topics_piece){
            ...
            if(in_array(sanitize_title($topics_piece),$the_tags_array)){
          ...
          foreach($topics_pieces as $topics_piece){
            ...
            if(in_array(strtolower($topics_piece),$the_tags_array)){
              $tags[]=$the_tags_name[strtolower($topics_piece)];
            }
          }
          //
          // $my_post = array(
    ...
    public function get_company_news(){
      ...
      global $wpdb;
      if(isset($_GET["cron"])){
        $results = $wpdb->get_results("SELECT * FROM `".$wpdb->postmeta."` WHERE `meta_key`='last_updated_news' AND `meta_value`<'".(time()-7*24*3600)."' ORDER BY `meta_value` ASC LIMIT 0,5");
      }else{
        $results = $wpdb->get_results("SELECT * FROM `".$wpdb->postmeta."` WHERE `meta_key`='last_updated_news' AND `meta_value`<'".(time()-7*24*3600)."' ORDER BY `meta_value` ASC");
      }
    ...
    function get_financials_info($pid){
      ...
      $ch = curl_init();
      echo "http://www.gdstudios.com/quote_media/company_financials_info.php?company=".get_post_meta($pid,"company_code",true);
      curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_financials_info.php?company=".get_post_meta($pid,"company_code",true));
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
      $output = curl_exec($ch);
      curl_close($ch);
      $json = json_decode($output,true);
    ...
    function get_filings_info($pid){
      ...
      $ch = curl_init();
      echo "http://www.gdstudios.com/quote_media/company_filings_info.php?company=".get_post_meta($pid,"company_code",true);
      curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_filings_info.php?company=".get_post_meta($pid,"company_code",true));
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
      $output = curl_exec($ch);
      curl_close($ch);
      $json = json_decode($output,true);
    ...
    function get_pricing_info($pid){
      ...
      $ch = curl_init();
      echo "http://www.gdstudios.com/quote_media/company_price_info.php?company=".get_post_meta($pid,"company_code",true);
      curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_price_info.php?company=".get_post_meta($pid,"company_code",true));
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
      $output = curl_exec($ch);
      curl_close($ch);
      $json = json_decode($output,true);
    ...
    function get_basic_info($pid){
      ...
      if($company_code){
        ...
        $ch = curl_init();
        echo "http://www.gdstudios.com/quote_media/company_basic_info.php?company=".$company_code;
        curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_basic_info.php?company=".$company_code);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $output = curl_exec($ch);
        curl_close($ch);
        $json = json_decode($output,true);
    ...
    function get_snap_price_info($pid){
      ...
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_snap_price.php?company_codes=".$company_codes);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
      $output = curl_exec($ch);
      curl_close($ch);
      $json = json_decode($output,true);
        ...
        foreach($json["quotedata"] as $price_info){
          ...
          foreach($ex_codes[$price_info["symbol"]] as $exc){
            ...
            if($price_info["pricedata"]["last"]){
              foreach($alerts["price_less"] as $it){
              ...
              foreach($alerts["price_more"] as $it){
                if($price_info["pricedata"]["last"] > $it["value"]){
                  $wpdb->query("UPDATE `wp_alerts` SET `active`='0' WHERE `id`='".$it["alert_id"]."'");
                  $wpdb->query("INSERT INTO `wp_alerts_triggered` SET `alert_id`='".$it["alert_id"]."', `time`='".time()."', `value`='".$price_info["pricedata"]["last"]."'");
                }
            ...
            if($price_info["pricedata"]["tradevolume"]){
              foreach($alerts["tradevolume_less"] as $it){
                if($it["value"] > $price_info["pricedata"]["tradevolume"]){
                  $wpdb->query("UPDATE `wp_alerts` SET `active`='0' WHERE `id`='".$it["alert_id"]."'");
                  $wpdb->query("INSERT INTO `wp_alerts_triggered` SET `alert_id`='".$it["alert_id"]."', `time`='".time()."', `value`='".$price_info["pricedata"]["tradevolume"]."'");
                }
                if($price_info["pricedata"]["tradevolume"] > $it["value"]){
                  if($it["active"]==0){
                    $wpdb->query("UPDATE `wp_alerts` SET `active`='1' WHERE `id`='".$it["alert_id"]."'");
                  }
                }
              }
              foreach($alerts["tradevolume_more"] as $it){
                if($price_info["pricedata"]["tradevolume"] > $it["value"]){
                  $wpdb->query("UPDATE `wp_alerts` SET `active`='0' WHERE `id`='".$it["alert_id"]."'");
                  $wpdb->query("INSERT INTO `wp_alerts_triggered` SET `alert_id`='".$it["alert_id"]."', `time`='".time()."', `value`='".$price_info["pricedata"]["tradevolume"]."'");
                }
    ...
    public function get_companies(){
      ...
      foreach($exchanges as $ex){
        ...
        if(isset($_GET["exchange"]) && $ex_code==$_GET["exchange"]){
          $ch = curl_init();
          curl_setopt($ch, CURLOPT_URL, "http://www.gdstudios.com/quote_media/company_list.php?exchange=".$ex_code);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          $output = curl_exec($ch);
          curl_close($ch);
          //print_r($output);
